...
static void nsvg__flattenShape(NSVGrasterizer* r, NSVGshape* shape, float scale)
{
  ...
  for (path = shape->paths; path != NULL; path = path->next)
  {
    ...
    for (i = 0; i < path->npts - 1; i += 3)
    {
      ...
      nsvg__flattenCubicBez(r, p[0] * scale, p[1] * scale, p[2] * scale, p[3] * scale, p[4] * scale,
                            p[5] * scale, p[6] * scale, p[7] * scale, 0, 0);
    }
    // Close path
    nsvg__addPathPoint(r, path->pts[0] * scale, path->pts[1] * scale, 0);
    // Build edges
...
static void nsvg__flattenShapeStroke(NSVGrasterizer* r, NSVGshape* shape, float scale)
{
  ...
  for (path = shape->paths; path != NULL; path = path->next)
  {
    ...
    for (i = 0; i < path->npts - 1; i += 3)
    {
      float* p = &path->pts[i * 2];
      nsvg__flattenCubicBez(r, p[0] * scale, p[1] * scale, p[2] * scale, p[3] * scale, p[4] * scale,
                            p[5] * scale, p[6] * scale, p[7] * scale, 0, NSVG_PT_CORNER);
    }
    if (r->npoints < 2)
      continue;
                                       ...
                                       NSVGcachedPaint* cache, char fillRule)
{
  ...
  for (y = 0; y < r->height; y++)
  {
    ...
    if (xmin <= xmax)
    {
      nsvg__scanlineSolid(&r->bitmap[y * r->stride] + xmin * 4, xmax - xmin + 1, &r->scanline[xmin],
                          xmin, y, tx, ty, scale, cache);
    }
  }
}
...
static void dumpEdges(NSVGrasterizer* r, const char* name)
{
  ...
  fprintf(fp, "<svg viewBox=\"%f %f %f %f\" xmlns=\"http://www.w3.org/2000/svg\">", xmin, ymin,
(xmax - xmin), (ymax - ymin));
...
r->points[i].x, r->points[i].y, r->points[i+1].x, r->points[i+1].y); fprintf(fp ,"<circle cx=\"%f\"
cy=\"%f\" r=\"1\" style=\"fill:%s;\" />", r->points[i].x, r->points[i].y, r->points[i].flags == 0 ?
"#f00" : "#0f0");
  }
  ...
  for (shape = image->shapes; shape != NULL; shape = shape->next)
  {
    ...
    for (j = 0; j < 3; j++)
    {
      ...
      if (paintOrder == NSVG_PAINT_FILL && shape->fill.type != NSVG_PAINT_NONE)
      {
        ...
        // now, traverse the scanlines and find the intersections on each scanline, use non-zero
        // rule
